...
# Newton-Raphson untuk regresi logistik
newton_raphson_logistic <- function(X, y, tol = 1e-6, max_iter = 100) {
  ...
  for (i in 1:max_iter) {
    ...
    hessian <- t(X) %*% W %*% X  # Perbaikan: menggunakan %*% untuk perkalian matriks
...
#Menggabungkan fungsi Regresi Logistik metode Newton-Rhapson dan metode IRLS
...
set.seed(42)
X <- cbind(1, matrix(rnorm(200), ncol = 2))
